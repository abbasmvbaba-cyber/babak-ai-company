---
employee_id: BAC-0012
full_name: پرنیا نادری
department: بازاریابی
role: کارشناس شبکه‌های اجتماعی
gender: زن
age_bucket: ۲۰ تا ۲۴ سال
synthetic_demo: true
---

# پرونده پرسنلی پرنیا نادری

> **دادهٔ کاملاً ساختگی / غیرقابل استفاده رسمی** — این پرونده برای تست رابط و گردش‌کار HR ساخته شده است. کد ملی، عکس و مدارک واقعی نیستند.

## اطلاعات شناسایی

| فیلد | مقدار |
|---|---|
| نام و نام خانوادگی | پرنیا نادری |
| جنسیت | زن |
| تاریخ تولد | 1381/02/07 |
| سن در تاریخ 1405/06/12 | 24 سال |
| نام پدر | بهرام |
| کد ملی نمایشی | IR-DEMO-000012 |
| محل تولد | ساری |
| تابعیت | ایرانی |

## اطلاعات شغلی

| فیلد | مقدار |
|---|---|
| شماره پرسنلی | BAC-0012 |
| دپارتمان | بازاریابی |
| تیم Agency-in-a-BOX | Marketing |
| سمت | کارشناس شبکه‌های اجتماعی |
| تاریخ استخدام | 1400/01/04 |
| سابقه تقریبی در شرکت | 5 سال |
| نوع همکاری | تمام‌وقت — دادهٔ نمونه |
| وضعیت | فعال — نمونه |
| ایمیل سازمانی نمونه | employee012@babak-ai.company |
| داخلی نمونه | 1012 |
| سرپرست مستقیم | الینا کاظمی |
| پایه حقوق ماهانه | ۳۹,۰۰۰,۰۰۰ |
| واحد پول | تومان |

## سوابق تحصیلی

| مقطع | رشته | مؤسسه | تاریخ شروع | تاریخ اخذ | وضعیت |
|---|---|---|---|---|---|
| دیپلم | ریاضی و فیزیک | دبیرستان نمونه ایرانیان | 1396/07/01 | 1399/03/15 | اخذ شده |
| کارشناسی | بازاریابی و ارتباطات | دانشگاه نمونه ایرانیان | 1399/07/01 | 1403/06/20 | اخذ شده |
| کارشناسی ارشد | — | — | — | ندارد | ندارد |
| دکتری | — | — | — | ندارد | ندارد |

## وضعیت خانوادگی

| فیلد | مقدار |
|---|---|
| وضعیت تأهل | مجرد |
| نام همسر | ندارد |
| تعداد فرزندان | 0 |

## اسناد موجود در این پوشه

- [تصویر پروفایل نمونه](photo.svg)
- [خلاصه استخدام و حکم داخلی نمونه](documents/employment-record.md)
- [سوابق تحصیلی نمونه](documents/education-record.md)
- [خلاصه جبران خدمات نمونه](documents/compensation-record.md)
- [فرم وضعیت خانوادگی نمونه](documents/family-record.md)
- [چک‌لیست مدارک مورد نیاز](documents/document-checklist.md)

## یادداشت HR

این پرونده تا زمان جایگزینی داده‌های واقعی در سامانهٔ خصوصی، صرفاً برای تست است. مدارک هویتی، قرارداد نهایی، بیمه و گواهی‌های رسمی عمداً به صورت «بارگذاری نشده» نگه داشته شده‌اند.
